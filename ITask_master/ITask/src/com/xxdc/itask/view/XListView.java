...
public class XListView extends ListView implements OnScrollListener {
	...
	// the interface to trigger(引发) refresh and load more.
	...
	public void stopRefresh() {
		...
		try {
			...
			int pos = count - itemCount;
			if (pos > 0) {
				setSelectionFromTop(pos, firstItemHeight);
			}
		} catch (Exception e) {
	...
	/**
	 * stop load more, reset footer view. 上拉加载更多之后,滑动到底部
	 */
	public void stopLoadMore() {
		if (mPullLoading == true) {
			mPullLoading = false;
			mFooterView.setState(XListViewFooter.STATE_NORMAL);
			setSelection(getBottom());//滑动到底部
		}
	}
				...
				if (mEnablePullRefresh && mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
					...
					new Timer().schedule(new TimerTask() {
						...
						public void run() {
							...
							if (getChildCount() > 2) { // 头尾各一个，这里表示有数据
								firstItemHeight = getChildAt(2).getTop(); // 等头部高度重置后计算一下位置
							}
						}
					}, SCROLL_DURATION);
